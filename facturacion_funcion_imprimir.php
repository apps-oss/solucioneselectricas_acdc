...
function print_ticket($id_factura)
{
    ...
    if ($nrows_fact>0) {
        ...
        //dividir texto giro
        $desgiro = $align->wordwrap1("GIRO: ".$giro_suc, 65);
        $descgiro="";
        ...
        if ($rowDesc!=null) {
            $cantidad =1;
            $descfinal    = $align->addspright(DESCUENTO, 36);
            $pre  = $align->rightaligner(number_format($rowDesc['precio_venta'], 2, ".", ","), $espacio, 8);
            $cant = $align->rightaligner($cantidad, $espacio, 4);
            $subt = $align->rightaligner(number_format($rowDesc['subtotal'], 2, ".", ","), $espacio, 9);
            $det_ticket .= $descfinal." " . $cant  ." ". $pre . " " . $subt . "\n";

            $total_final= $total_final -$subt;
            $total_gravado = $total_gravado -$subt;
        }
        $det_ticket .= FONT_A;
        $det_ticket .= chr(13) . $line1;
...
function print_fact($id_factura, $nitcte='', $nombreapecte='')
{
    ...
    $info_factura.=  LINEINCH8; // 12/203  1,6,12,18 etc
        ...
        if ($datos_extra!="" || !is_string($datos_extra)) {
         ...
         $rowDesc=getDetalleDescuento($id_factura, $id_sucursal);
         if ($rowDesc!=null) {
             $subt = number_format(round($rowDesc['subtotal'], 2), 2);
             $pv   = number_format(round($rowDesc['precio_venta'], 2), 2);
             ...
             $info_factura.= $align->addspleft($cantidad, $cb_arr[1])." "; //cantidad
             $info_factura.= $align->addspright(" ", $cb_arr[2]);//espacio
             $info_factura.= $align->addspright($descripcion, $lendesc); //descripcion
             $info_factura.= $align->addspleft($pv, $cb_arr[4]);  //precio
             $info_factura.= $align->addspleft("", $cb_arr[5]);  //vta. no sujetas
             $info_factura.= $align->addspleft("", $cb_arr[6]);  //vta. exenta
             $info_factura.= $align->addspleft($subt, $cb_arr[7])."\n"; //subtotal
