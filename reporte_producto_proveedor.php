...
class PDF extends FPDF
{
    ...
    public function __construct($orientation = 'P', $unit = 'mm', $format = 'letter')
    {
        ...
        $this->report_name = "Set the report name with setReportName";
        $this->subtitle = "";
    }
...
$query = 'SELECT * FROM producto WHERE id_proveedor = ' . $id_proveedor;
...
$cantidadTotal[$monthsCount + 1];
$subtotalTotal = 0;
$subtotalConIvaTotal = 0;
...
while ($row = _fetch_array($productos)) {
    ...
    foreach ($months as $i => $item) {
        ...
        $query  = "SELECT SUM(cantidad) as cantidad, SUM(subtotal) as subtotal";
        $query .= " FROM factura_detalle WHERE id_prod_serv = ";
        $query .= $row['id_producto'] . " AND MONTH(fecha) = " . $month;
        $query .= " AND YEAR(fecha) = " . $year;

        $RESULT = _query($query);

        $row_detail = _fetch_assoc($RESULT);
        $data[$key][] = (float)$row_detail['cantidad'];

        $cantidad += (float)$row_detail['cantidad'];
        ...
        $subtotalTotal += (float)$row_detail['subtotal'];
        $subtotalConIvaTotal += (float)$row_detail['subtotal'] * 1.13;

        $totalIva += (float)$row_detail['subtotal'] * 1.13;
        $totalSinIva += (float)$row_detail['subtotal'];
    }

    $fecha = strtotime($firstDate);
    $month_key = 0;
    ...
    $data[$key][] = '$ ' . number_format($totalSinIva, 2);
    $data[$key][] = '$ ' . number_format($totalIva, 2);
...
foreach ($cantidadTotal as $item) {
    $data[$key][] = $item;
}

$data[$key][] = '$ ' . number_format($subtotalTotal, 2);
$data[$key][] = '$ ' . number_format($subtotalConIvaTotal, 2);
...
$header = [];
$header[] = $pdf->th('Descripcion', 'C', 25);


foreach ($months as $item) {
    $header[] = $pdf->th($monthStrings[(int)$item['month']], 'C', 11);
}

$header[] = $pdf->th('Total sin IVA', 'C', 20);
$header[] = $pdf->th('Total con IVA', 'C', 20);
