...
function deleted()
{
    ...
    $q="SELECT movimiento_producto.id_movimiento,factura.total,factura.fecha
	FROM factura JOIN movimiento_producto ON movimiento_producto.id_factura=factura.id_factura
  ...
  $table="movimiento_stock_ubicacion";
    ...
    //restar galones mes si cliente tiene DIF
    $tablec="fact_imp_combust";
    $table_dif = 'consumo_mes_dif';
    $sql_fc = "SELECT id,  id_impuesto, id_cliente,
 total_imp,imp_nombre,id_dif,galones_dif
 FROM $tablec
 WHERE id_factura='$id_factura'
 AND galones_dif>0";
    $q_fc = _query($sql_fc);
    $nrow_fc = _num_rows($q_fc);

    if ($nrow_fc>0) {
        $row_fc = _fetch_array($q_fc);
        $id_dif= $row_fc['id_dif'];
        $id_cliente= $row_fc['id_cliente'];
        $galones_dif= $row_fc['galones_dif'];
        $form_datac = array(
         'anulada' => 1,
     );

        $wc_fic="id_factura='".$id_factura."'";
        $upd_fic = _update($tablec, $form_datac, $wc_fic);
        list($year, $month, $day)=explode("-", $fecha_fact);
        //restar de consumo mensual
        $sql2 = "SELECT id_consumo_dif, cons_gal_mes
		FROM consumo_mes_dif
		WHERE id_dif='$id_dif'
		...
		";
        $res2 = _query($sql2);
        $nrows2=_num_rows($res2);

        if ($nrows2>0) {
            $row2           = _fetch_row($res2);
            $id_consumo_dif = $row2[0];
            $consumo        = $row2[1];
            $consumo_actual = $consumo - $galones_dif ;
            $fd_dif = array(
                'cons_gal_mes'  => $consumo_actual,
            );
            $wcdif="id_consumo_dif='".$id_consumo_dif."'";
            $upd_dif = _update($table_dif, $fd_dif, $wcdif);
        }
    }
    //fin anular GALONES CONSUMO MES DIF
    //eliminar abonos y creditos si existen
    $sql_DIF = _query("SELECT id_credito FROM credito WHERE id_factura=$id_factura");
