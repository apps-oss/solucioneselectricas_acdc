...
$sql=_query("SELECT Hoja1.cod_bar,Hoja1.Nombre,REPLACE(REPLACE(Hoja1.Costo_Iva,'$',''),',','.') AS costo,REPLACE(REPLACE(Hoja1.Precio1,'$',''),',','.') AS precio,proveedor.id_proveedor,categoria.id_categoria FROM Hoja1 JOIN proveedor ON proveedor.nombre=Hoja1.Casa JOIN categoria ON categoria.nombre_cat=Hoja1.Familia WHERE Hoja1.Nombre IS NOT NULL");
