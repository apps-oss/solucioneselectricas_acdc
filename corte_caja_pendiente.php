...
function initial() {
  ...
  $_PAGE ['title'] = $title;
	...
	$fecha_actual  = $row_apertura["fecha"];
	...
	<div class="wrapper wrapper-content  animated fadeInRight">
		<div class="row">
			<div class="col-lg-12">
				<div class="ibox ">
					...
					if ($links!='NOT' || $admin=='1' ){
						$row_aper= getDatosAperturaNoVigente($aper_id,$id_sucursal);
						...
						<div class="ibox-content">
							<form name="formulario" id="formulario">
								<div class="row">
									...
									<input type='text' class='form-control datepicker' id='fecha' name='fecha' value='$fecha_apertura' ></div>";
									echo "</div>";
									$url_lect="lectura_dia.php?id_apertura=$aper_id&fecha=$fecha_apertura&pend=1";
									?>
								</div>
								...
								<div class="col-lg-6" id="tipo_pagos">
									...
									<tbody>
										...
										<tr>
											<tr>
												<td>TOTAL CONSUMO INTERNO</td>
												<td><?php echo sprintf("%.2f",	$total_consumo_interno_2);?></td>
											</tr>
											<tr>
							...
							<div class='row'>
								...
								$url_lub="lectura_lub_dia.php?id_apertura=$id_apertura&pend=1";
								...
								<div class='row' style='margin-bottom:3px'>
									<a class="btn btn-md btn-success pull-right btn-block"
									 href='<?= $url_lub2;?>' 		id='btnPrintLect' target="_blank">
			...
			echo "<script src='js/funciones/corte_pendiente.js'></script>";
		}
		else {
	...
	function corte()
	{
		...
		function cambio()
		{
			...
			$fecha_actual = $row_apertura1["fecha"];
			...
			{
				$q = "SELECT MIN(numero_doc) as minimo, MAX(numero_doc) as maximo FROM factura
				WHERE fecha = '$fecha_apertura' AND credito=0 AND id_apertura_pagada = '$id_apertura'
				AND hora BETWEEN '$hora_apertura' AND '$hora_actual' AND numero_doc LIKE '%TIK%' AND id_sucursal = '$id_sucursal' AND anulada = 0
				UNION ALL SELECT MIN(CONVERT(num_fact_impresa,UNSIGNED IN$aper_idTEGER)) as minimo, MAX(CONVERT(num_fact_impresa,UNSIGNED INTEGER)) as maximo
				FROM factura WHERE fecha = '$fecha_apertura' AND credito=0 AND id_apertura_pagada = '$id_apertura'
				AND hora BETWEEN '$hora_apertura' AND '$hora_actual' AND numero_doc LIKE '%COF%' AND id_sucursal = '$id_sucursal' AND anulada = 0
				UNION ALL SELECT MIN(CONVERT(num_fact_impresa,UNSIGNED INTEGER)) as minimo,
				MAX(CONVERT(num_fact_impresa,UNSIGNED INTEGER)) as maximo FROM factura
				WHERE fecha = '$fecha_apertura' AND credito=0 AND id_apertura_pagada = '$id_apertura'
				AND hora BETWEEN '$hora_apertura' AND '$hora_actual' AND numero_doc LIKE '%CCF%' AND id_sucursal = '$id_sucursal' AND anulada = 0";
				$sql_min_max = _query($q);
				$cuenta_min_max = _num_rows($sql_min_max);
...
function getApertura(){
	...
	$row=getDatosAperturaNoVigente($id_apertura,$id_sucursal);
	$id_caja=$row['caja'];
