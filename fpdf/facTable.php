 ...
 class PDFact extends FPDF
 {
     ...
     public function Header()
     {
         ...
         if ($this->libase['mostrar']==1) {
             ...
             for ($i=1; $i < $ncol; $i++) {
                 $this->Line($this->libase["l$i"], 0, $this->libase["l$i"], 212);
             }

             //lineas encabezado
             for ($i=1; $i < 10; $i++) {
                 $this->Line(0, $this->libase["e$i"], 322, $this->libase["e$i"]);
             }
             for ($i=1; $i < 5; $i++) {
                 // $this->Line(20, $this->libase["e$i"], 280, $this->libase["e$i"]);

                 $this->Line(($this->libase["linvert"]["l$i"]), 52, $this->libase["linvert"]["l$i"], 245);
                 // $this->Line(20, 45, 20, 153);
             }
             /*

             //limite de impresion izquierdo por cada factura
             for ($i=0; $i < $ncol; $i++) {
                 $this->Line(($this->libase[$this->libase["imll$i"]["b"]])+($this->libase["imll$i"]["p"]), 0, ($this->libase[$this->libase["imll$i"]["b"]])+($this->libase["imll$i"]["p"]), 212);
             }
             //limite de impresion derecho por cada factura
             for ($i=1; $i < $ncol; $i++) {
                 $this->Line(($this->libase[$this->libase["imlr$i"]["b"]])-($this->libase["imlr$i"]["m"]), 0, ($this->libase[$this->libase["imlr$i"]["b"]])-($this->libase["imlr$i"]["m"]), 212);
             }
             //lineas de footer
             for ($i=1; $i < 9; $i++) {
                 $this->Line(0, $this->libase["f$i"], 322, $this->libase["f$i"]);
             }

             //limite de impresion footer por cada factura
             for ($i=1; $i <$ncol; $i++) {
                 $this->Line(($this->libase[$this->libase["imlf$i"]["b"]])-($this->libase["imlf$i"]["m"]), $this->libase["e7"], ($this->libase[$this->libase["imlf$i"]["b"]])-($this->libase["imlf$i"]["m"]), 212);
             }*/
         }
     }
