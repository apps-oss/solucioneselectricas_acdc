$(document).ready(function() {
  ...
  $('#notacred1').on('ifChecked', function(event){
    $("#ajuste").show();
    $("#devolucion").hide();
  });
  $('#notacred2').on('ifChecked', function(event){
  ...
  $(document).on('click', '#btnGuardar', function(event) {
    ...
    $('#tabla tr').each(function(index) {
      if(index>0)
      {
        ...
        boni = parseInt($(this).find('#boni').val());
        if(isNaN(valor))
        {
          valor=0;
        }
...
//btnGuardarAjuste
$(document).on('click', '#btnGuardarAjuste', function(event) {
  let suma=0;
  let valor=0;
  let id_factura=$('#id_factura').val();
  let numero_doc=$('#numero_doc').val();
  let id_cliente=$('#id_cliente').val();
  let apertura=$('#id_apertura').val();
  let caja=$('#caja').val();
  let turno=$('#turno').val();
  let tipo =$('#tipo_doc').val();
  let total_facturado  =$('#total_facturado').val();
  let concepto   = $('#concepto').val();
  let concepto2  = $('#concepto2').val();
  let concepto3  = $('#concepto3').val();
  let concepto4  = $('#concepto4').val();
  let concepto5  = $('#concepto5').val();
  let valor_ajuste  =$('#valor_ajuste').val();

  if(isNaN(valor_ajuste))
  {
    valor_ajuste=0;
  }
  let dataString =  "process=ajuste&id_factura="+id_factura+"&numero_doc="+numero_doc
  dataString += "&id_cliente="+id_cliente+"&tipo="+tipo+"&caja="+caja
  dataString += "&apertura="+apertura+"&turno="+turno;
  dataString += "&total_facturado="+total_facturado+"&valor_ajuste="+valor_ajuste+"&concepto="+concepto;
  dataString += "&concepto2="+concepto2+"&concepto3="+concepto3+"&concepto4="+concepto4+"&concepto5="+concepto5;

  if( parseFloat(valor_ajuste) > 0 && parseFloat(valor_ajuste) <= parseFloat(total_facturado) && concepto!="")
  {
    $.ajax({
      type: 'POST',
      url: 'notacredito.php',
  ...
  {
    display_notify('Error','Total Devolución debe ser  mayor que cero y menor que Total Facturado;\n El concepto no debe ir vacio!');
  }
});
