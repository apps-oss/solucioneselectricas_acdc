...
function actualiza_subtotal(tr) {
  ...
  let id_producto =tr.find('.id_pps').text()
  ...
  /*
  if (tipo_impresion != "CCF"  && id_producto!='-999') {
...
function addProductList(id_proda, tip)
{
  ...
  $.ajax({
    ...
    success: function(data)
        ...
        let tr_add = new_tr(id_prod,filas,descripcion,es_exento,tipo_bien,existencias,cantidades, select,select_rank,preciop_s_iva,descripcionp,preciop, btnView, 0, unidades)
        ...
        numItems = countItems()
        tr_act=$('#inventable tr:nth-child('+numItems+ ')') .closest("tr")
        actualiza_subtotal(tr_act)
	      setTotals();
	      scrolltable();
    	}
			else
...
let totalColumns=()=>{
  ...
  $("#inventable tr").each(function() {
    ...
    total         = parseFloat(total_exento) + parseFloat(total_gravado);
    console.log("total_gravado:"+total_gravado)
    total_con_desc =  total -total_descuento
   let totaless ={
    totalcant       : totalcant,
...
let new_tr=(id_prod,filas,descripcion,es_exento,tipo_bien,existencias,cantidades, select,select_rank,preciop_s_iva,descripcionp,preciop, btnView, subtotal=0, unidades = 1)=>{
  let tr_add ="";
  tr_add += "<tr  class='row100 head' id='" + filas + "'>";
        tr_add += "<td hidden class='cell100 column10 text-success id_pps'>"
        tr_add +="<input type='hidden' id='unidades' name='unidades' value='" + unidades + "'>" + id_prod+ es_exento+tipo_bien + "</td>";
