...
$direccion = Mayu(utf8_decode(trim($row_emp["direccion"])));
    ...
    */
    $set_x = $pdf-> GetX();
    $pdf->SetXY($set_x, $set_y);
    $pdf->Cell(57, 15, utf8_decode("NOMBRE DEL CLIENTE"), 1, 0, 'C');
    ...
    $set_x = $pdf-> GetX();
    $pdf->SetXY($set_x, $set_y);
    $pdf->Cell(117, 5, utf8_decode("OPERACIONES DE VENTAS PROPIAS Y A CUENTA DE TERCEROS"), 1, 1, 'C');
    $set_y=$set_y+5;

    $pdf->SetXY($set_x, $set_y);
    $pdf->Cell(78, 3, utf8_decode("PROPIAS"), 1, 1, 'C');
    $pdf->SetX($set_x);
    $pdf->Cell(12, 3, utf8_decode("NO"), "LR", 1, 'C');
    $pdf->SetX($set_x);
    $pdf->Cell(12, 4, utf8_decode("SUJETAS"), "BLR", 0, 'C');
    $set_x = $pdf-> GetX();
    $pdf->SetXY($set_x, $pdf->GetY()-3);
    $pdf->Cell(12, 7, utf8_decode("EXENTAS"), 1, 0, 'C');
    $set_x = $pdf-> GetX();
    $pdf->SetX($set_x);
    ...
    $set_x = $pdf-> GetX();
    $pdf->SetXY($set_x, $pdf->GetY()-3);
    $pdf->Cell(15, 3, utf8_decode("DEBITO"), "LR", 1, 'C');
    $pdf->SetX($set_x);
    $pdf->Cell(15, 4, utf8_decode("FISCAL"), "BLR", 0, 'C');

    $set_x = $pdf->GetX();
    $pdf->SetXY($set_x, $pdf->GetY()-3);
    $pdf->Cell(12, 3, utf8_decode("FOVIAL"), "LTR", 1, 'C');
    $pdf->SetX($set_x);
    $pdf->Cell(12, 4, utf8_decode("-"), "BLR", 0, 'C');

    $set_x = $pdf->GetX();
    $pdf->SetXY($set_x, $pdf->GetY()-3);
    $pdf->Cell(12, 3, utf8_decode("COTRANS"), "LTR", 1, 'C');
    $pdf->SetX($set_x);
    $pdf->Cell(12, 4, utf8_decode("-"), "BLR", 0, 'C');
    ...
    $q="SELECT factura.id_factura,factura.tipo_documento,factura.fecha,factura.num_fact_impresa,
    factura.sumas,
    factura.serie, cliente.nombre,cliente.nrc,factura.iva as total_iva,
    factura.retencion AS total_retencion,factura.total,factura.anulada
    FROM factura JOIN cliente ON cliente.id_cliente=factura.id_cliente
    WHERE factura.fecha BETWEEN '".$_REQUEST[fini]."' AND '".$_REQUEST[ffin]."' AND tipo_documento='CCF' AND factura.id_sucursal=$id_sucursal  ";
    $sql=_query($q);
    $nrows=_num_rows($sql);
    $i=1;
    $pa=1;
        ...
        while ($row=_fetch_array($sql)) {
            if ($c==32&&$pa==1) {
                ...
                */
                ...
                $pdf->Cell(12, 5, utf8_decode(number_format(0.00, 2)), "T", 0, 'R');
                $pdf->Cell(12, 5, utf8_decode(number_format(0.00, 2)), "T", 0, 'R');
            ...
            if ($row['anulada']==1) {
                # code...
                $pdf->Cell(57, 5, utf8_decode("<<COMPROBANTE ANULADO>>"), 0, 0, 'L');

                $pdf->Cell(10, 5, utf8_decode(""), 0, 0, 'L');

                $pdf->Cell(12, 5, utf8_decode(number_format(0.00, 2)), 0, 0, 'R');
                ...
                # code...
                $pdf->Cell(57, 5, utf8_decode($row['nombre']), 0, 0, 'L');
                ...
                $pdf->Cell(15, 5, utf8_decode(number_format($row['sumas'], 2)), 0, 0, 'R');

                $total_internas_gravadas=$total_internas_gravadas+(round(($row['sumas']), 2));

                $pdf->Cell(15, 5, utf8_decode(number_format(($row['total_iva']), 2)), 0, 0, 'R');

                //impuestos a la gasolina

                $impuestoGas=getImpGass($row['id_factura']);
                ...
                if($n_imp>0){
                  for ($n=0;$n<$n_imp;$n++){
                    ...
                    $imp_nombre = $row_imp['imp_nombre'];
                     $imp_nombre = $row_imp['imp_nombre'];
                    if( $imp_nombre=='FOVIAL' ) {
                      //$tot_imp = 0;
                      $tot_imp = $row_imp['total_imp'];
                      $pdf->Cell(12, 5, number_format(($tot_imp), 2), 0, 0, 'R');
                      $total_fovial += $tot_imp;
                    }
                    if( $imp_nombre=='COTRANS' ) {
                      //$tot_imp = 0;
                      $tot_imp = $row_imp['total_imp'];
                      $pdf->Cell(12, 5, number_format(($tot_imp), 2), 0, 0, 'R');
                      $total_cotrans += $tot_imp;
                    }
                  }
               }
            ...
            if ($c==32 && $pasa == 0) {
                ...
                $pdf->Cell(15, 5, utf8_decode(number_format($total_internas_gravadas, 2)), "T", 0, 'R');
                $pdf->Cell(15, 5, utf8_decode(number_format($total_debito_fiscal, 2)), "T", 0, 'R');
                $pdf->Cell(12, 5, utf8_decode(number_format($total_fovial, 2)), "T", 0, 'R');
                $pdf->Cell(12, 5, utf8_decode(number_format($total_cotrans, 2)), "T", 0, 'R');
                $pdf->Cell(13, 5, utf8_decode(number_format(0.00, 2)), "T", 0, 'R');
                $pdf->Cell(13, 5, utf8_decode(number_format(0.00, 2)), "T", 0, 'R');
                $pdf->Cell(13, 5, utf8_decode(number_format(0.00, 2)), "T", 0, 'R');
                $pdf->Cell(10, 5, utf8_decode(number_format($total_retencion, 2)), "T", 0, 'R');
